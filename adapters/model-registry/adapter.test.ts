...
describe('registryStatusSummary / getModelRegistry', () => {
  it('reports 4 UNAVAILABLE entries and 1 SHADOW entry (laya-typed)', () => {
    const summary = registryStatusSummary();
    expect(summary).toHaveLength(5);
    const byId = Object.fromEntries(summary.map((s) => [s.model_id, s.status]));
    expect(byId['laya-typed']).toBe('SHADOW');
    expect(['laya-english', 'laya-multilingual', 'jev', 'open-jev'].every((id) => byId[id] === 'UNAVAILABLE')).toBe(true);
  });
